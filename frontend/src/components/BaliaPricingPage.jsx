                ...
                <div className="mt-3 pl-4 border-l-2 border-amber-300 space-y-2">
                  ...
                  {model.heaterVariants.map((hv, idx) => (
                    <div key={idx} className="flex items-center gap-4 flex-wrap bg-amber-50 p-2 rounded">
                      <span className="text-sm">{hv.type === 'integrated' ? (lang === 'ru' ? 'Встроенная' : 'Zintegrowany') : hv.type === 'none' ? (lang === 'ru' ? 'Без печи' : 'Bez pieca') : hv.type === 'electric' ? (lang === 'ru' ? 'Электрическая' : 'Elektryczny') : (lang === 'ru' ? 'Внешняя' : 'Zewnętrzny')}</span>
                      <div className="flex items-center gap-2">
                        <Label className="text-xs text-muted-foreground">ID:</Label>
                        <Input
