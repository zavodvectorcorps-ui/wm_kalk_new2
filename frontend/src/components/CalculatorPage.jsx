    ...
    <>
      <div className="mb-3 p-2 bg-blue-50/50 rounded border border-blue-100">
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            {hint && (
              <p className="text-sm text-gray-600 whitespace-pre-line">{hint}</p>
            )}
            {hasMedia && (
              <button 
                onClick={() => setDialogOpen(true)}
              ...
              {prices.categories?.filter(category => {
                ...
                <div key={category.id} className="border-b pb-4 last:border-b-0">
                  ...
                  {(category.displayType === 'tiles' || category.displayType === 'grid') ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      {filterColorOptions(category, filterOptionsByBowlType(category, category.options || [])).filter(option => {
