      ...
      {model ? (
        <>
          ...
          <div className="p-3 bg-amber-50 rounded-lg border border-amber-200">
            ...
            {/* Room sizes from selected variant */}
            {(roomSizes?.relaxRoomSize || roomSizes?.steamRoomSize || roomSizes?.terraceSize || roomSizes?.entranceSide || roomSizes?.capacity) && (
              <div className="mt-2 pt-2 border-t border-amber-200 text-xs space-y-1">
                {roomSizes.capacity && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Liczba osób:</span>
                    <span className="font-medium text-amber-800">{roomSizes.capacity}</span>
                  </div>
                )}
                {roomSizes.terraceSize && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Taras:</span>
                    <span className="font-medium text-amber-800">{roomSizes.terraceSize}</span>
                  </div>
                )}
                {roomSizes.relaxRoomSize && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Pokój wyp.:</span>
                    <span className="font-medium text-amber-800">{roomSizes.relaxRoomSize}</span>
                  </div>
                )}
                {roomSizes.steamRoomSize && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Sauna:</span>
                    <span className="font-medium text-amber-800">{roomSizes.steamRoomSize}</span>
                  </div>
                )}
                {roomSizes.entranceSide && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Wejście:</span>
                    <span className="font-medium text-amber-800">{roomSizes.entranceSide}</span>
                  </div>
                )}
                {roomSizes.hasTerrace && !roomSizes.terraceSize && (
...
const SelectedOptionsList = ({ prices, formData, getCategoryName, isOptionVisible, adminGifts = [], toggleGift, removeOption, canGiveGifts, txt }) => {
