...
import { 
  ...
  Percent, Calculator, Tag, Mail, X, Edit, Gift, Shield, Package, Info, Play, Image as ImageIcon
} from 'lucide-react';
...
export const SaunaCalculator = ({ editingOrder = null, onEditComplete, amocrmPrefill = null, onAmocrmPrefillUsed = null }) => {
  const {
    ...
    handleQuantityChange, handleSubOptionChange, handleSaveAndGeneratePDF, handleClearForm,
...
const RadioOptions = ({ category, options, formData, handleRadioChange, handleQuantityChange, handleSubOptionChange, txt }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
    {options.map((option) => {
      ...
      return (
        <div key={option.id} className="space-y-2">
          <div className={`relative flex items-start space-x-3 p-3 rounded-lg border transition-all cursor-pointer ${isSelected ? 'bg-amber-50 border-amber-400' : 'bg-muted/30 border-border hover:bg-muted/50'}`} onClick={() => handleRadioChange(category.id, option.id)}>
            {/* Hint icon with media support */}
            <HintIcon 
              hint={option.hint} 
              hintImageUrl={option.hintImageUrl} 
          ...
          {/* Sub-options - show only when main option is selected */}
          {isSelected && hasSubOptions && (
            <div className="ml-8 pl-3 border-l-2 border-purple-300 space-y-1">
              {option.subOptions.map((subOpt) => {
                ...
                const isSubChecked = formData.subSelections?.[subOptKey] || false;
                return (
                  <div key={subOpt.id} className={`flex items-center gap-2 p-2 rounded ${isSubChecked ? 'bg-purple-50' : 'bg-gray-50'}`} onClick={(e) => e.stopPropagation()}>
                    <CheckboxOrange 
                      id={subOptKey}
                      checked={isSubChecked}
                      onCheckedChange={(checked) => handleSubOptionChange(option.id, subOpt.id, checked)}
