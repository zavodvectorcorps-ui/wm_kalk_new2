...
`https://sauna-order-sync.preview.emergentagent.com`

#### Вариант B: Добавление кнопки через цифровую воронку
1. В настройках этапа воронки добавьте **Webhook**
2. URL: `https://sauna-order-sync.preview.emergentagent.com/?calc=balia&amocrm_id={{lead.id}}`
    ...
    this.callbacks = {
      ...
      leads: {
        selected: function() {
          ...
          var app_url = 'https://sauna-order-sync.preview.emergentagent.com';
...
### Balia калькулятор:
```
https://sauna-order-sync.preview.emergentagent.com/?calc=balia&amocrm_id=ID_СДЕЛКИ
```

### Sauna калькулятор:
```
https://sauna-order-sync.preview.emergentagent.com/?calc=sauna&amocrm_id=ID_СДЕЛКИ
```
